...
orclHelper.prototype.executeRows = function (sql) {
	var enumerator = argEnumerator(arguments);
	var params = enumerator.next('object') || {};
	var cb = enumerator.next('function') || orclHelper._noop;
	return this._execute(sql, params, function (data) {
		cb(data.rows);
	});
};
...
orclHelper.prototype.tables = function (cb) {
	return this.executeRows('select table_name from user_tables', function (data) {
...
orclHelper.prototype.columns = function (table, cb) {
	return this.executeRows('select column_name, data_type, data_length from user_tab_cols where table_name=:0', [table], function (rows) {
		cb(rows.map(function (row) {
			return row.concat(ctype('orc', row[1]));
		}));
	});
};
